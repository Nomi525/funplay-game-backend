...
export class UserController {
  ...
  @Post('/signup-signin-otp')
  @UsePipes(ValidationPipe)
  public async userSignUpSignInOtp(
    @Req() req: Request,
    @Res() res: Response,
    @Body() body: UserSignUpSignInOtpRequestDto,
  ): Promise<UserSignUpSignInOtpResponseDto | []> {
    Logger.access.info(
      'user.controller --> info of userSignUpSignInOtp() for login and register users',
    );
    try {
      const signupSigningOTPDetails = await this.userService.signupSigningOTP(
        req,
        res,
        body,
      );
      return signupSigningOTPDetails;
    } catch (error) {
      Logger.error.error(
        'user.controller --> userSignUpSignInOtp() indicates error',
        error.message,
      );
  ...
  public async getProfile(
    ...
    Logger.access.info('user.controller --> info of getProfile()');
    try {
      const getSingleUserProfile = await this.userService.getUserProfile(
        req,
        res,
      );
      return getSingleUserProfile;
    } catch (error) {
      console.log({ error });
      Logger.error.error(
        'user.controller --> getProfile() indicates error',
        error.message,
      );
      throw new BadRequestException(error.message);
    }
